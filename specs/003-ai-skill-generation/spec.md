...
As a skill creator, I want to describe my skill requirements in natural language and have AI generate a complete skill file so that I can create skills faster than manual writing.
...
**Independent Test**: Can be tested by invoking AI generation with a prompt, watching streaming content appear, and applying the generated content to a new file.
...
4. **Given** the generated content is in the editor, **When** the user saves, **Then** the skill file is created with valid structure
...
4. **Given** the modified content is applied, **When** the user saves, **Then** the file is updated with changes

---

### User Story 3 - Insert Content at Cursor (Priority: P2)

As a skill author, I want AI to generate and insert content at a specific location in my skill file so that I can build skills incrementally.
...
3. **Given** content is inserted, **When** the user saves, **Then** the file reflects the insertion
...
3. **Given** the replacement is applied, **When** the user saves, **Then** the file reflects the change
...
- **FR-004**: System MUST encrypt and store AI API keys using secure storage (Electron safeStorage or OS credential manager)
- **FR-005**: System MUST provide real-time preview of AI-generated content before applying to editor
- **FR-006**: Users MUST be able to stop AI generation mid-stream and preserve partial content
- **FR-007**: System MUST validate AI-generated YAML frontmatter and display errors before allowing apply
- **FR-008**: System MUST support retry and regenerate operations with same or modified prompts
- **FR-009**: System MUST enforce reasonable prompt length limits and display character counter
- **FR-010**: System MUST handle AI service errors (rate limits, network failures, invalid credentials) with actionable error messages
- **FR-011**: System MUST allow configuration of AI provider, API key, model selection, and streaming toggle
- **FR-012**: System MUST provide connection test functionality to validate AI credentials
- **FR-013**: System MUST display generation timeout prompt after 30 seconds with continue/cancel options
- **FR-014**: System MUST highlight differences between original and AI-modified content for review
- **FR-015**: System MUST integrate AI assist button in skill editor toolbar with keyboard shortcut (Ctrl+G)
