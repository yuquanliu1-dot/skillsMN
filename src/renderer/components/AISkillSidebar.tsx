...
export const AISkillSidebar: React.FC<AISkillSidebarProps> = ({
  ...
  return (
    <div className="fixed right-0 top-0 h-full w-96 bg-white shadow-2xl z-50 flex flex-col border-l border-slate-200 animate-slide-in">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-600 via-violet-600 to-indigo-600 px-4 py-4 flex items-center justify-between flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
        {messages.length === 0 && (
          <div className="text-center text-slate-400 py-12">
            ...
            <p className="text-sm">Start a conversation to create or modify skills</p>
            <p className="text-xs mt-2 text-slate-300">AI will help you write skill.md files</p>
          </div>
        )}
      ...
      <div className="p-4 bg-white border-t border-slate-200 flex-shrink-0">
        <div className="relative">
          <textarea
            ...
            className="w-full px-4 py-3 pr-12 border border-slate-300 rounded-xl resize-none focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:bg-slate-100 disabled:cursor-not-allowed text-sm bg-white text-slate-900 placeholder-slate-400"
            rows={3}
            maxLength={2000}
          />
          <div className="absolute bottom-2 right-2 text-xs text-slate-400">{inputValue.length}/2000</div>
        </div>

        {/* Action buttons */}
        <div className="flex items-center justify-between mt-3">
          <div className="text-xs text-slate-400">
            <kbd className="px-1.5 py-0.5 bg-slate-100 rounded">Enter</kbd> to send
          </div>
          <div className="flex gap-2">
            {isStreaming ? (
              ...
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-lg transition-colors flex items-center gap-2 text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
