  ...
  return (
    <div data-testid="private-repos-list" className="h-full flex flex-col">
      ...
      <div className="border-b border-gray-200 p-4 space-y-3 flex-shrink-0 bg-white">
        ...
        {/* Bottom row: Repository selector + README button + Tag filter + Skills count + Sort */}
      ...
      {/* README Dialog */}
      {showReadmeDialog && selectedRepo && (
        <ReadmeDialog
          repoId={selectedRepo.id}
          repoName={selectedRepo.displayName || `${selectedRepo.owner}/${selectedRepo.repo}`}
          onClose={() => setShowReadmeDialog(false)}
        />
      )}
    </div>
  );
