...
describe('SkillDiscovery', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        skillDiscovery = new skillDiscovery_1.SkillDiscovery();
        // Mock path.join
    ...
    describe('findSkillByName()', () => {
        it('should find skill directory by exact name match', async () => {
            ...
            fs.promises.readdir.mockResolvedValueOnce(mockEntries);
        ...
        it('should respect maxDepth parameter', async () => {
            ...
            fs.promises.readdir.mockResolvedValue(rootEntries);
        ...
        it('should return undefined if SKILL.md not found in matched directory', async () => {
            ...
            fs.promises.readdir.mockResolvedValueOnce(mockEntries);
            fs.existsSync.mockReturnValueOnce(false); // SKILL.md not found
            const result = await skillDiscovery.findSkillByName(repoDir, skillName);
            expect(result).toBeUndefined();
        });
    ...
    describe('writeSourceMetadata()', () => {
        it('should write .source.json file with metadata', async () => {
            ...
            expect(fs.promises.writeFile).toHaveBeenCalledWith('/test/skills/my-skill/.source.json', JSON.stringify(sourceMetadata, null, 2), 'utf-8');
        });
        ...
        it('should write metadata without commit hash', async () => {
            ...
            fs.promises.writeFile = jest.fn().mockResolvedValue(undefined);
            await skillDiscovery.writeSourceMetadata(skillDir, sourceMetadata);
            expect(fs.promises.writeFile).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('"type": "registry"'), 'utf-8');
        });
    });
...
//# sourceMappingURL=skillDiscovery.test.js.map