...
describe('GitOperations', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        gitOperations = new gitOperations_1.GitOperations();
    });
    ...
    describe('shallowClone()', () => {
        it('should successfully clone a repository with shallow depth', async () => {
            ...
            expect(mockExecAsync).toHaveBeenCalledWith(expect.stringContaining('git clone --depth 1 --single-branch'), expect.any(Object));
        });
        it('should return error when git is not available', async () => {
            ...
            expect(result.error).toBe(gitOperations_1.ERROR_MESSAGES.GIT_NOT_FOUND.user);
        ...
        it('should detect repository not found error', async () => {
            ...
            // Mock clone failure
            mockExecAsync.mockRejectedValueOnce(new Error('fatal: repository \'https://github.com/owner/nonexistent-repo.git\' not found'));
            const result = await gitOperations.shallowClone(source, targetDir);
            expect(result.success).toBe(false);
            expect(result.error).toBe(gitOperations_1.ERROR_MESSAGES.REPO_NOT_FOUND.user);
        ...
        it('should detect private repository error', async () => {
            ...
            mockExecAsync.mockRejectedValueOnce(new Error('fatal: Authentication failed for \'https://github.com/owner/private-repo.git/\''));
            const result = await gitOperations.shallowClone(source, targetDir);
            expect(result.success).toBe(false);
            expect(result.error).toBe(gitOperations_1.ERROR_MESSAGES.PRIVATE_REPO.user);
        ...
        it('should detect network error', async () => {
            ...
            mockExecAsync.mockRejectedValueOnce(new Error('fatal: unable to access \'https://github.com/owner/repo.git/\': Failed to connect to github.com'));
            const result = await gitOperations.shallowClone(source, targetDir);
            expect(result.success).toBe(false);
    ...
    describe('parseGitError()', () => {
        it('should parse git not found error', () => {
            ...
            const result = gitOperations.parseGitError(error);
            expect(result.code).toBe('GIT_NOT_FOUND');
            expect(result.message).toBe(gitOperations_1.ERROR_MESSAGES.GIT_NOT_FOUND.user);
        });
        it('should parse repository not found error (404)', () => {
            ...
            expect(result.message).toBe(gitOperations_1.ERROR_MESSAGES.REPO_NOT_FOUND.user);
        });
        it('should parse private repository error (403)', () => {
            ...
            expect(result.message).toBe(gitOperations_1.ERROR_MESSAGES.PRIVATE_REPO.user);
        });
        it('should parse network error', () => {
            const error = 'fatal: unable to access \'https://github.com/\': Failed to connect';
            const result = gitOperations.parseGitError(error);
            expect(result.code).toBe('NETWORK_ERROR');
            expect(result.message).toBe(gitOperations_1.ERROR_MESSAGES.NETWORK_ERROR.user);
        });
        it('should return clone failed error for unknown git errors', () => {
            ...
            expect(result.message).toBe(gitOperations_1.ERROR_MESSAGES.CLONE_FAILED.user);
        });
    });
    describe('ERROR_MESSAGES constant', () => {
        it('should have all required error codes', () => {
            expect(gitOperations_1.ERROR_MESSAGES.GIT_NOT_FOUND).toBeDefined();
            expect(gitOperations_1.ERROR_MESSAGES.REPO_NOT_FOUND).toBeDefined();
            expect(gitOperations_1.ERROR_MESSAGES.PRIVATE_REPO).toBeDefined();
            expect(gitOperations_1.ERROR_MESSAGES.NETWORK_ERROR).toBeDefined();
            expect(gitOperations_1.ERROR_MESSAGES.DISK_SPACE_ERROR).toBeDefined();
            expect(gitOperations_1.ERROR_MESSAGES.CLONE_FAILED).toBeDefined();
        });
        it('should have user-friendly messages', () => {
            expect(gitOperations_1.ERROR_MESSAGES.GIT_NOT_FOUND.user).toContain('Git');
            expect(gitOperations_1.ERROR_MESSAGES.GIT_NOT_FOUND.action).toContain('git-scm.com');
            expect(gitOperations_1.ERROR_MESSAGES.REPO_NOT_FOUND.user).toContain('repository');
            expect(gitOperations_1.ERROR_MESSAGES.REPO_NOT_FOUND.action).toContain('alternative');
        });
    });
});
//# sourceMappingURL=gitOperations.test.js.map