...
`;
            ...
            const result = await skillInstaller.installFromRegistry(request, skillsDir, (event) => progressMessages.push(event.message));
    ...
    describe('Cross-Cutting: Security', () => {
        it('should sanitize skill names to prevent path traversal', async () => {
            ...
            for (const name of maliciousNames) {
                const slugified = skillInstaller.slugify(name);
        ...
        it('should use HTTPS for all external communication', async () => {
            ...
            expect(fetch).toHaveBeenCalledWith(expect.stringMatching(/^https:/), expect.any(Object));
        });
    });
});
//# sourceMappingURL=registry-e2e-workflows.test.js.map